...
#include "pqStringVectorPropertyWidget2.h"
#include "vtkSMCompoundSourceProxy2.h"
#include "vtkSMDocumentation2.h"
#include "vtkSMDoubleVectorProperty2.h"
#include "vtkSMIntVectorProperty2.h"
#include "vtkSMNamedPropertyIterator2.h"
#include "vtkSMOrderedPropertyIterator2.h"
#include "vtkSMPropertyGroup2.h"
#include "vtkSMProxyListDomain2.h"
#include "vtkSMProxyProperty.h"
#include "vtkSMSettings2.h"
#include "vtkSMStringVectorProperty2.h"
#include "vtkSMTrace2.h"
*/
#include <QHideEvent>
#include <QLabel>
#include <QPointer>
...
QWidget* newGroupSeparator(QWidget* parent)
{
  ...
  // b vbox->setContentsMargins(0, 0, 0, pqPropertiesPanel::suggestedVerticalSpacing());
  vbox->setSpacing(0);
...
public:
  ...
  {
    if (show_advanced == false) // b  && this->isAdvanced(proxy) == true)
    {
...
public:
  ...
  pqInternals(vtkSMProxy* smproxy,  QStringList properties) : Proxy(smproxy), CachedShowAdvanced(false)
  {
    /*
  ...
  , Internals(new pqProxyWidget::pqInternals(smproxy, properties))
{
  ...
  // b this->setEnabled(pqApplicationCore::instance()->getActiveServer()->isMaster());
...
{
  // b return (smproxy && smproxy->GetHints() && smproxy->GetHints()->FindNestedElementByName("UseDocumentationForLabels"));
  return false;
}
...
{
    /* b
  const char* xmlDocumentation = smProperty ? vtkGetDocumentation(smProperty->GetDocumentation(), dtype) : nullptr;
...
{
  // b SM_SCOPED_TRACE(PropertiesModified).arg("proxy", this->proxy());
  Q_FOREACH (const pqProxyWidgetItem* item, this->Internals->Items)
  {
    item->apply();
  }
...
void pqProxyWidget::createWidgets(const QStringList& properties)
{
  ...
  // b vtkVLogScopeF(PARAVIEW_LOG_APPLICATION_VERBOSITY(), "creating widgets for `%s`", smproxy->GetLogNameOrDefault());
...
bool pqProxyWidget::filterWidgets(bool show_advanced, const QString& filterText)
{
  ...
  Q_FOREACH (const pqProxyWidgetItem* item, this->Internals->Items)
  {
    ...
    if (visible)
    {
      // b item->show(prevItem, item->enableWidget(), show_advanced);
      prevItem = item;
    }
    else
    {
...
bool pqProxyWidget::restoreDefaults()
{
  ...
  if (this->Internals->Proxy)
  {
    ...
    // b iter.TakeReference(this->Internals->Proxy->NewPropertyIterator());
