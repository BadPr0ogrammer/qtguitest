...
/* b
#include "pqActiveObjects2.h"
#include "pqDataRepresentation2.h"
#include "pqExtractor2.h"
#include "pqLiveInsituManager2.h"
#include "pqServerManagerModel2.h"
#include "pqSettings2.h"
#include "pqUndoStack2.h"
#include "vtkPVGeneralSettings2.h"
#include "vtkSMProxyClipboard2.h"
#include "vtkSMViewProxy2.h"
#include "vtkTimerLog2.h"
*/
#include <QKeyEvent>
#include <QPointer>
#include <QStyle>
...
{
  if (proxy) // b  && pqLiveInsituManager::isInsitu(proxy))
  {
    return false;
  }
...
public:
    ...
    : Superclass(panel)
  {
    ...
    // b this->Panel->setView(panel->view());
...
public:
  ...
  // b QPointer<pqView> View;
  QPointer<pqProxy> Source;
  int SourcePort;
  // b QPointer<pqDataRepresentation> Representation;
  ...
  , PanelMode(pqPropertiesPanel::ALL_PROPERTIES)
{
  ...
  this->connect(activeObjects, SIGNAL(pipelineProxyChanged(pqProxy*)), this, SLOT(setPipelineProxy(pqProxy*)));
  this->connect(activeObjects, SIGNAL(viewChanged(pqView*)), this, SLOT(setView(pqView*)));
  this->connect(activeObjects, SIGNAL(representationChanged(pqDataRepresentation*)), this, SLOT(setRepresentation(pqDataRepresentation*)));
  ...
  this->connect(smm, SIGNAL(modifiedStateChanged(pqServerManagerModelItem*)), SLOT(updateButtonState()));
  this->connect(pqApplicationCore::instance()->getServerManagerModel(), SIGNAL(connectionRemoved(pqPipelineSource*, pqPipelineSource*, int)), SLOT(updateButtonState()));
  this->connect(pqApplicationCore::instance()->getServerManagerModel(), SIGNAL(connectionAdded(pqPipelineSource*, pqPipelineSource*, int)), SLOT(updateButtonState()));
  */
  ...
  // b this->setView(nullptr);
  // b this->setRepresentation(nullptr);
}
...
void pqPropertiesPanel::updatePanel()
{
  ...
  // b this->updateDisplayPanel(internals.Representation);
  // b this->updateViewPanel(internals.View);
  this->updateButtonState();
}
...
{
  // b this->updateDisplayPanel(this->Internals->Representation);
}
/* b
//-----------------------------------------------------------------------------
...
*/
//-----------------------------------------------------------------------------
void pqPropertiesPanel::sourcePropertyChanged(bool change_finished) // b =true)
{
  /* b
...
void pqPropertiesPanel::updateButtonEnableState()
{
  ...
  if (internals.Source)
  {
    ...
    // b ui.PropertiesPaste->setEnabled(internals.SourceClipboard->CanPaste(internals.Source->getProxy()));
  }
  else
  {
...
{
  // b bool onlyApplyCurrentPanel = vtkPVGeneralSettings::GetInstance()->GetAutoApplyActiveOnly();
  // b if (onlyApplyCurrentPanel)
  {
    pqProxyWidgets* widgets =
      this->Internals->Source ? this->Internals->SourceWidgets[this->Internals->Source] : nullptr;
    if (widgets)
...
void pqPropertiesPanel::deleteProxy()
{
  ...
  {
    // b BEGIN_UNDO_SET(tr("Delete") + " " + source->getSMName());
...
{
  // b this->Internals->SourceClipboard->Copy(this->Internals->Source->getProxy());
  this->updateButtonEnableState();
}
...
{
  // b this->Internals->SourceClipboard->Paste(this->Internals->Source->getProxy());
  // b this->renderActiveView();
}

//-----------------------------------------------------------------------------
void pqPropertiesPanel::copyDisplay()
{
  // b this->Internals->DisplayClipboard->Copy(this->Internals->Representation->getProxy());
  this->updateButtonEnableState();
}
...
{
  // b this->Internals->DisplayClipboard->Paste(this->Internals->Representation->getProxy());
  // b this->renderActiveView();
}
...
{
  // b this->Internals->ViewClipboard->Copy(this->Internals->View->getProxy());
  this->updateButtonEnableState();
}
...
{
  // b this->Internals->ViewClipboard->Paste(this->Internals->View->getProxy());
  // b this->renderActiveView();
}
