...
public:
  pqInternals(pqSearchBox* self)
  {
    ...
    self->connect(this->SearchLineEdit, SIGNAL(textChanged(QString)), self, SIGNAL(textChanged(QString)));
    self->connect(this->AdvancedButton, SIGNAL(clicked(bool)), self, SLOT(onAdvancedButtonClicked(bool)));
    // b self->connect(self->settings(), SIGNAL(modified()), self, SLOT(updateFromSettings()));
  };
};
