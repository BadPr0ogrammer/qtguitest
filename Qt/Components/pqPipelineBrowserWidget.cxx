...
pqPipelineBrowserWidget::pqPipelineBrowserWidget(QWidget* parentObject)
  ...
  // b , PipelineModel(
  // b new pqPipelineModel(*pqApplicationCore::instance()->getServerManagerModel(), this))
  // b , FilteredPipelineModel(new pqPipelineAnnotationFilterModel(this))
  // b , ContextMenu(new QMenu(this)
  
{
  this->configureModel();

  // Initialize pqFlatTreeView.
  // b this->Superclass::setModel(this->FilteredPipelineModel);
  this->getHeader()->hide();
  ...
  QObject::connect(this, SIGNAL(clicked(const QModelIndex&)), this, SLOT(handleIndexClicked(const QModelIndex&)));
  // b QObject::connect(&pqActiveObjects::instance(), SIGNAL(viewChanged(pqView*)), this, SLOT(setActiveView(pqView*)));

  // b new pqPipelineModelSelectionAdaptor(this->getSelectionModel());
}
...
bool pqPipelineBrowserWidget::viewportEvent(QEvent* evt)
{
  if (evt->type() == QEvent::FontChange)
  {
    ...
    QFont modifiedFont = this->font();
    modifiedFont.setBold(true);
    // b this->PipelineModel->setModifiedFont(modifiedFont);
  }
...
{
  // b this->FilteredPipelineModel->setAnnotationFilterMatching(matching);
}

//----------------------------------------------------------------------------
void pqPipelineBrowserWidget::enableAnnotationFilter(const QString& annotationKey)
{
  // b this->FilteredPipelineModel->enableAnnotationFilter(annotationKey);
}

//----------------------------------------------------------------------------
void pqPipelineBrowserWidget::disableAnnotationFilter()
{
  // b this->FilteredPipelineModel->disableAnnotationFilter();
}
//----------------------------------------------------------------------------
/* b
...
{
  // b this->FilteredPipelineModel->disableSessionFilter();
}
...
{
  // b this->expand(this->FilteredPipelineModel->mapFromSource(index));
}
