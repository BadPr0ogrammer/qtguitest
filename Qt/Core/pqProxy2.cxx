...
#include "pqServerManagerObserver2.h"
...
//-----------------------------------------------------------------------------
pqProxy::pqProxy(const QString& group, const QString& name, vtkSMProxy* proxy, 
    // b pqServer* server,
...
pqProxy::~pqProxy()
{
  ...
  // b pqServerManagerObserver* observer = core->getServerManagerObserver();
  // b QObject::disconnect(observer, SIGNAL(proxyRegistered(const QString&, const QString&, vtkSMProxy*)), this, SLOT(onProxyRegistered(const QString&, const QString&, vtkSMProxy*)));
  // b QObject::disconnect(observer, SIGNAL(proxyUnRegistered(const QString&, const QString&, vtkSMProxy*)), this, SLOT(onProxyUnRegistered(const QString&, const QString&, vtkSMProxy*)));
...
void pqProxy::addHelperProxy(const QString& key, vtkSMProxy* proxy)
{
  ...
  if (!already_added)
  {
    ...
    // b QString groupname = QString("pq_helper_proxies.%1").arg(this->getProxy()->GetGlobalIDAsString());

    // b vtkSMSessionProxyManager* pxm = this->proxyManager();
    // b pxm->RegisterProxy(groupname.toUtf8().data(), key.toUtf8().data(), proxy);
  }
}
