...
// b #include "vtkSMMessageMinimal2.h"            // needed for vtkSMMessage
// b #include "vtkSMDomainIterator2.h"            // needed for vtkSMDomainIterator

class vtkPVXMLElement;
class vtkSMProxy;
// b struct vtkSMPropertyInternals;
/* b
class vtkSMDomain;
...
public:
   ...
   * @endcode
   */
  // b template <class DomainType>
  // b inline DomainType* FindDomain();
   ...
   */
  // b void AddDomain(const char* name, vtkSMDomain* dom);

  /**
   * Add a link to a property whose value should be synchronized
   * with this property value.
   */
  // b virtual void AddLinkedProperty(vtkSMProperty* targetProperty);

  /**
   * Remove a link to a property added with AddLinkedProperty()
   */
  // b virtual void RemoveLinkedProperty(vtkSMProperty* targetProperty);
   ...
   */
  // b virtual void RemoveFromSourceLink();
   ...
   * if applicable.
   */
  // b virtual void Copy(vtkSMProperty* src);
   ...
   * for this property.
   */
  // b vtkGetObjectMacro(Documentation, vtkSMDocumentation);
  //@}
   ...
   */
  // b virtual bool ResetToDomainDefaults(bool use_unchecked_values = false);
  ...
  // b void SetHints(vtkPVXMLElement* hints);
  //@}
...
protected:
  ...
  // b friend class vtkSMSessionProxyManager;
  friend class vtkSMProxy;
  friend class vtkSMSubPropertyIterator;
  // b friend class vtkSMDomainIterator;
  friend class vtkSMSourceProxy;
  // b friend class vtkSMDomain;
  // b friend class vtkSMPropertyModificationUndoElement;

  /**
   * Let the property write its content into the stream
   */
  // b virtual void WriteTo(vtkSMMessage* msg);

  /**
   * Let the property read and set its content from the stream
   */
  // b virtual void ReadFrom(const vtkSMMessage*, int vtkNotUsed(message_offset), vtkSMProxyLocator*){};

  /**
   * Set the appropriate ivars from the xml element. Should
   * be overwritten by subclass if adding ivars.
   */
  // b virtual int ReadXMLAttributes(vtkSMProxy* parent, vtkPVXMLElement* element);
   ...
   */
  // b void AddDependent(vtkSMDomain* dom);
   ...
   */
  // b virtual void SaveState(vtkPVXMLElement* parent, const char* property_name, const char* uid, int saveDomains = 1);
  /**
   * This method must be overridden by concrete class in order to save the real
   * property data
   */
  // b virtual void SaveStateValues(vtkPVXMLElement* propertyElement);

  /**
   * Save property domain
   */
  // b virtual void SaveDomainState(vtkPVXMLElement* propertyElement, const char* uid);

  /**
   * Updates state from an XML element. Returns 0 on failure.
   */
  // b virtual int LoadState(vtkPVXMLElement* element, vtkSMProxyLocator* loader);

  vtkPVXMLElement* Hints;

  char* Command;

  // b vtkSMPropertyInternals* PInternals;
  ...
  // b vtkSMDomainIterator* DomainIterator;
  ...
  // b vtkSMInformationHelper* InformationHelper;

  // b void SetInformationProperty(vtkSMProperty* ip);
  vtkSMProperty* InformationProperty;

  // b vtkSMDocumentation* Documentation;
  // b void SetDocumentation(vtkSMDocumentation*);
  ...
  // b void SetParent(vtkSMProxy* proxy);
