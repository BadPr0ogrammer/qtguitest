...
/*
#include "vtkClientServerStream2.h"
#include "vtkDataSetAttributes2.h"
#include "vtkPVAlgorithmPortsInformation2.h"
#include "vtkPVArrayInformation2.h"
#include "vtkPVDataInformation2.h"
#include "vtkPVDataSetAttributesInformation2.h"
#include "vtkProcessModule2.h"
#include "vtkSMDocumentation2.h"
#include "vtkSMDoubleVectorProperty2.h"
#include "vtkSMIdTypeVectorProperty2.h"
#include "vtkSMInputProperty2.h"
#include "vtkSMIntVectorProperty2.h"
#include "vtkSMMessage2.h"
#include "vtkSMOutputPort2.h"
#include "vtkSMProxyLocator2.h"
#include "vtkSMSession2.h"
#include "vtkSMStringVectorProperty2.h"
*/
#include <cassert>
#include <sstream>
#include <string>
...
{
  // b this->SetSIClassName("vtkSISourceProxy");
...
vtkTypeUInt32 vtkSMSourceProxy::GetGlobalID()
{
  ...
  // b if (!has_gid && this->Session != nullptr)
  {
    // reserve 1+MAX_NUMBER_OF_PORTS contiguous IDs for the source proxies and possible extract
    // selection proxies.
    // b this->SetGlobalID(this->GetSession()->GetNextChunkGlobalUniqueIdentifier(1 + MAX_NUMBER_OF_PORTS));
  }
...
{
  // b if (this->ObjectsCreated)
  {
    // b vtkClientServerStream stream;
    // b stream << vtkClientServerStream::Invoke << SIPROXY(this) << "UpdatePipelineInformation" << vtkClientServerStream::End;
    // b this->ExecuteStream(stream);
  }

  // This simply iterates over subproxies and calls UpdatePropertyInformation();
  // b this->Superclass::UpdatePipelineInformation();
...
int vtkSMSourceProxy::ReadXMLAttributes(vtkSMSessionProxyManager* pm, vtkPVXMLElement* element)
{
  ...
  // b return this->Superclass::ReadXMLAttributes(pm, element);
  return 0;
}
...
void vtkSMSourceProxy::UpdatePipeline()
{
  ...
  for (int i = 0; i < num; ++i)
  {
    // b this->GetOutputPort(i)->UpdatePipeline();
  }
...
void vtkSMSourceProxy::UpdatePipeline(double time)
{
  ...
  for (i = 0; i < num; ++i)
  {
    // b this->GetOutputPort(i)->UpdatePipeline(time);
  }
...
void vtkSMSourceProxy::CreateVTKObjects()
{
  ...
  // b this->Superclass::CreateVTKObjects();
...
unsigned int vtkSMSourceProxy::GetNumberOfAlgorithmOutputPorts()
{
  ...
  {
    /* b 
    vtkSmartPointer<vtkPVAlgorithmPortsInformation> info = vtkSmartPointer<vtkPVAlgorithmPortsInformation>::New();
...
void vtkSMSourceProxy::PostUpdateData(bool using_cache)
{
  ...
  // b this->Superclass::PostUpdateData(using_cache);
}
...
void vtkSMSourceProxy::MarkDirty(vtkSMProxy* modifiedProxy)
{
  ...
  // b this->Superclass::MarkDirty(modifiedProxy);
  // this->InvalidateDataInformation();
}
...
vtkPVDataInformation* vtkSMSourceProxy::GetDataInformation(unsigned int idx)
{
  ...
  // b return this->GetOutputPort(idx)->GetDataInformation();
}
...
vtkPVDataInformation* vtkSMSourceProxy::GetRankDataInformation(unsigned int idx, int rank)
{
  ...
  // b return this->GetOutputPort(idx)->GetRankDataInformation(rank);
}
  ...
  unsigned int idx, const char* selector, const char* assemblyName)
{
  ...
  // b return this->GetOutputPort(idx)->GetSubsetDataInformation(selector, assemblyName);
}
  ...
  unsigned int idx, unsigned int compositeIndex)
{
  ...
  // b return this->GetOutputPort(idx)->GetSubsetDataInformation(compositeIndex);
}
...
void vtkSMSourceProxy::InvalidateDataInformation()
{
  if (this->OutputPortsCreated)
  {
    ...
    {
      // b it->Port.GetPointer()->InvalidateDataInformation();
    }
  }
}
...
{
  // b this->Superclass::SetLogNameInternal(name, propagate_to_subproxies, propagate_to_proxylistdomains);
  ...
  for (size_t port = 0, max = internals.SelectionProxies.size(); port < max; ++port)
  {
    if (auto esProxy = internals.SelectionProxies[port])
    {
      ...
      // b esProxy->SetLogName(stream.str().c_str());
    }
  }
}
